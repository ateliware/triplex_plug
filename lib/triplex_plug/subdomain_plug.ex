defmodule TriplexPlug.SubdomainPlug do
  @moduledoc """
  ...
  To plug it on your router, you can use:
      ...
      plug TriplexPlug.SubdomainPlug,
        endpoint: MyApp.Endpoint,
        tenant_handler: &TenantHelper.tenant_handler/1

  See `TriplexPlug.SubdomainPlugConfig` to check all the allowed `config` flags.
  """

  alias Plug.Conn

  alias TriplexPlug.SubdomainPlugConfig

  @doc false
  def init(opts), do: struct(SubdomainPlugConfig, opts)

  @doc false
  def call(conn, config), do: TriplexPlug.put_tenant(conn, get_subdomain(conn, config), config)
